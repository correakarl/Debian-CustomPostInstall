#!/bin/bash
# =============================================================================
# DEBIAN 13 (TRIXIE) - OMNI-OPTIMIZER & THEME ENGINE v8.5
...
#
# MANTENIMIENTO:
#   • Logs: .runtime-logs/debian-postinstall-*.log (rotación: últimos 2)
...
readonly LOG_DIR="${SCRIPT_DIR}/.runtime-logs"
readonly LOGFILE="${LOG_DIR}/debian-postinstall-$(date +%F-%H%M).log"
readonly STATUSFILE="${LOG_DIR}/debian-postinstall-status.log"
readonly RUN_STARTED_AT="$(date '+%F %T %z')"
readonly RUN_STARTED_EPOCH="$(date +%s)"
readonly BACKUP_DIR="/root/.omni-backup"
readonly AUTO_PROFILE_JSON="${SCRIPT_DIR}/config/autogenerated-system-profile.json"
...
AUTO_CONFIRM=false
PROFILE_JSON=""
APP_CATALOG_JSON="${SCRIPT_DIR}/config/app-library.json"
GPU_PROFILE="auto"
GPU_PURPOSE="auto"
PRE_HOOKS_DONE=false
POST_HOOKS_DONE=false
OPERATION_EXECUTED=false
RUN_LOG_FINALIZED=false
...
# Libreria de catalogo unificado (obligatoria para V1/V2).
if [[ -f "${SCRIPT_DIR}/lib/app-catalog.sh" ]]; then
    # shellcheck source=lib/app-catalog.sh
    source "${SCRIPT_DIR}/lib/app-catalog.sh"
fi
...
# -----------------------------------------------------------------------------
mkdir -p "$LOG_DIR" || {
    echo -e "${RED}Error: No se pudo crear directorio de logs en ${LOG_DIR}${NC}" >&2
    exit 1
}
exec > >(tee -a "$LOGFILE") 2>&1
echo "[RUN-START][V1] ts=${RUN_STARTED_AT} pid=$$ log=${LOGFILE}"
...
is_flatpak_installed() { flatpak list --app --columns=application 2>/dev/null | grep -Fxq "$1"; }
...
install_with_progress() {
    ...
    if ! apt_candidate_available_v1 "$pkg"; then
        if pkg_seen_in_apt_history_v1 "$pkg"; then
            log_status "skip" "[REPO:MISSING] $desc (detectado en history.log; repo/fuente previa no disponible hoy)"
        else
            log_status "skip" "[REPO:MISSING] $desc (posible repo externo no configurado)"
        fi
        return 0
    fi
    
    if show_spinner "Instalando $desc..." run_apt_with_repair apt install -y "$pkg"; then
        log_status "ok" "$desc"
        return 0
    else
...
repair_package_manager() {
    log_msg "${YELLOW}Detectado fallo APT/DPKG. Intentando autoreparación...${NC}"

    if fuser /var/lib/dpkg/lock-frontend >/dev/null 2>&1 || \
       fuser /var/lib/dpkg/lock >/dev/null 2>&1 || \
       fuser /var/cache/apt/archives/lock >/dev/null 2>&1; then
        log_status "fail" "Hay locks activos de APT/DPKG. Cierre otra instancia y reintente"
        return 1
    fi
    ...
    if dpkg --audit | grep -q .; then
        log_status "fail" "Autoreparación incompleta: quedan paquetes en estado inconsistente"
        return 1
    fi

    log_status "ok" "Autoreparación APT/DPKG completada"
    return 0
}
...
run_apt_with_repair() {
    ...
    repair_package_manager || return 1
    "$@" &>/dev/null
}
...
apt_candidate_available_v1() {
    local pkg="$1"
    local base_pkg="${pkg%:i386}"
    apt-cache show "$base_pkg" &>/dev/null
}

pkg_seen_in_apt_history_v1() {
    local pkg="$1"
    local base_pkg="${pkg%:i386}"
...
flatpak_app_available_v1() {
    local app_id="$1"
    cmd_exists "flatpak" || return 1
    flatpak remotes --columns=name 2>/dev/null | grep -qx 'flathub' || return 1
    flatpak remote-info flathub "$app_id" &>/dev/null
}
...
install_flatpak_with_progress() {
    ...
    if ! flatpak_app_available_v1 "$app_id"; then
        log_status "skip" "[FLATPAK:MISSING] $app_name (remoto/fuente no disponible)"
        return 0
    fi
...
validate_environment() {
    ...
    # 4. Permisos de escritura en directorio de logs del proyecto
    local log_write_test="${LOG_DIR}/.write-test.$$"
    if ! touch "$log_write_test" 2>/dev/null; then
        log_msg "${RED}Error: Sin permisos de escritura en ${LOG_DIR}${NC}"
        ((errors++))
    else
        rm -f "$log_write_test"
    fi
...
show_help() {
    cat <<EOF
OMNI-OPTIMIZER v8.5 - Post-instalación resiliente para Debian 13
...
OPCIONES:
  ...
  -y, --auto        Confirmar automáticamente prompts
    ...
    --catalog-json    Biblioteca JSON de apps/fuentes/duplicados
...
EJEMPLOS:
  ...
  sudo ./post-install.sh -v -y              # Auto + verbose
    ...
    sudo ./post-install.sh --catalog-json ./config/app-library.json --gpu-profile auto --gpu-purpose gaming

NOTAS:
    • Logs: ${SCRIPT_DIR}/.runtime-logs/debian-postinstall-*.log
    • Estado: ${SCRIPT_DIR}/.runtime-logs/debian-postinstall-status.log
...
setup_critical_infra() {
    ...
    # Actualizar índices
    if run_apt_with_repair apt update; then
...
pre_install_hooks() {
    if [[ "${PRE_HOOKS_DONE}" == "true" ]]; then
        log_status "skip" "Hooks PRE ya ejecutados para esta operacion"
        return 0
    fi
    ...
    if ! is_apt_installed "flatpak"; then
        if run_apt_with_repair apt install -y flatpak; then
...
post_install_hooks() {
    if [[ "${PRE_HOOKS_DONE}" != "true" ]]; then
        log_status "skip" "Hooks POST omitidos: no hubo fase PRE en esta operacion"
        return 0
    fi

    if [[ "${POST_HOOKS_DONE}" == "true" ]]; then
        log_status "skip" "Hooks POST ya ejecutados para esta operacion"
        return 0
    fi
...
rotate_logs() {
    local keep=2
    local logs=($(ls -t "${LOG_DIR}"/debian-postinstall-*.log 2>/dev/null))
    ...
    local states=($(ls -t "${LOG_DIR}"/debian-postinstall-status*.log 2>/dev/null))
    [[ ${#states[@]} -gt 1 ]] && rm -f "${states[@]:1}" 2>/dev/null
}

trim_status_file() {
    local max_lines=4000
    [[ -f "$STATUSFILE" ]] || return 0

    local line_count
    line_count=$(wc -l < "$STATUSFILE" 2>/dev/null || echo 0)
    if [[ "$line_count" -le "$max_lines" ]]; then
        return 0
    fi

    tail -n "$max_lines" "$STATUSFILE" > "${STATUSFILE}.tmp" 2>/dev/null || return 0
    mv -f "${STATUSFILE}.tmp" "$STATUSFILE" 2>/dev/null || true
    log_status "ok" "Estado recortado a ultimas ${max_lines} lineas para ahorrar disco"
}

finalize_run_log_v1() {
    local exit_code="$1"
    [[ "${RUN_LOG_FINALIZED}" == "true" ]] && return 0
    RUN_LOG_FINALIZED=true

    local ended_at ended_epoch duration
    ended_at="$(date '+%F %T %z')"
    ended_epoch="$(date +%s)"
    duration=$((ended_epoch - RUN_STARTED_EPOCH))

    echo "[RUN-END][V1] ts=${ended_at} pid=$$ exit=${exit_code} duration_sec=${duration}"
}
...
readonly UNIVERSAL_BASE=(
    ...
    xfce4-goodies xfce4-power-manager xfce4-power-manager-plugins xfce4-screensaver
...
readonly MODULE_OFFICE=(evince okular thunderbird)
...
readonly MODULE_AI_ENVIRONMENTS=(python3 python3-venv python3-pip python3-dev python3-numpy python3-scipy python3-pandas python3-matplotlib python3-sklearn ipython3 python3-h5py python3-pil python3-tqdm python3-requests python3-yaml git git-lfs)
...
# Registro unificado de modulos V1 (orden/menu).
readonly -a V1_MODULE_IDS=(
    optimization browsers office multimedia dev_core dev_cli dev_web dev_containers
    dev_mobile communication virtualization hardware_drivers vpn_free design_graphic
    design_video design_3d gaming ai_environments windows_compat cybersec remote
    monitoring backup security
)
readonly -a V1_MODULE_LABELS=(
    "Optimización del Sistema" "Navegadores" "Ofimática" "Multimedia"
    "Dev Core" "Dev CLI" "Dev Web (VS Code)" "Dev Contenedores"
    "Dev Mobile" "Comunicación" "Virtualización" "Hardware/Drivers" "VPN Libre"
    "Diseño Gráfico" "Diseño Video" "Diseño 3D" "Gaming Nativo"
    "AI / ML / Agentes" "Compatibilidad Windows (Bottles/Wine)" "Ciberseguridad"
    "Acceso Remoto" "Monitoreo" "Backup" "Seguridad"
)
...
v1_snapshot_category_packages() {
    local category="$1"
    catalog_get_v1_category_apt "$APP_CATALOG_JSON" "$category"
}

v1_snapshot_category_flatpaks() {
    local category="$1"
    catalog_get_v1_category_flatpak "$APP_CATALOG_JSON" "$category"
}

show_auto_profile_snapshot_v1() {
    if declare -F json_profile_show_snapshot_summary >/dev/null; then
        section_header "[JSON] Snapshot previo autogenerado"
        json_profile_show_snapshot_summary "$AUTO_PROFILE_JSON"
    fi
}

update_auto_profile_snapshot_v1() {
    if ! declare -F json_profile_write_snapshot >/dev/null; then
        return 0
    fi

    local -a snapshot_categories=(optimization browsers office multimedia dev_core dev_cli dev_web dev_containers dev_mobile communication virtualization hardware_drivers vpn_free design_graphic design_video design_3d ai_environments gaming windows_compat cybersec remote monitoring backup security)
    json_profile_write_snapshot "$AUTO_PROFILE_JSON" "autogenerated-v1" "$USER_HOME" "post-install-v1" snapshot_categories v1_snapshot_category_packages v1_snapshot_category_flatpaks
    log_status "ok" "Snapshot JSON actualizado: $AUTO_PROFILE_JSON"
}
...
install_module() {
    ...
    run_catalog_category_actions_v1 "$module_name" "pre"

    if [[ ${#apt_packages[@]} -gt 0 ]]; then
        log_msg "[PLAN][APT][$module_name] ${apt_packages[*]}"
    fi
    local module_flatpaks
    module_flatpaks="$(v1_snapshot_category_flatpaks "$module_name")"
    if [[ -n "$module_flatpaks" ]]; then
        log_msg "[PLAN][FLATPAK][$module_name] $module_flatpaks"
    fi
    ...
    if cmd_exists "flatpak"; then
        flatpak remote-add --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo &>/dev/null
        for app_id in $module_flatpaks; do
    ...
    run_catalog_category_actions_v1 "$module_name" "post"
...
is_xfce_environment() {
    [[ "$DESKTOP_ENV" =~ [Xx][Ff][Cc][Ee] ]] && return 0
    is_apt_installed "xfce4-session" && return 0
    return 1
}

apply_desktop_environment_fixes() {
    section_header "[UX FIXES] Reparando inconsistencias de escritorio"

    if is_xfce_environment; then
        log_msg "Aplicando hardening de energia para XFCE..."
        ...
        if cmd_exists "xfconf-query"; then
            ...
            if [[ -n "$user_uid" ]]; then
                ...
                sudo -u "$SUDO_USER" env DBUS_SESSION_BUS_ADDRESS="$user_bus" \
                    xfconf-query -c xfce4-power-manager -p /xfce4-power-manager/presentation-mode -n -t bool -s true &>/dev/null || true
                sudo -u "$SUDO_USER" env DBUS_SESSION_BUS_ADDRESS="$user_bus" \
                    xfconf-query -c xfce4-power-manager -p /xfce4-power-manager/lock-screen-suspend-hibernate -n -t bool -s false &>/dev/null || true
                sudo -u "$SUDO_USER" env DBUS_SESSION_BUS_ADDRESS="$user_bus" \
                    xfconf-query -c xfce4-screensaver -p /saver/enabled -n -t bool -s false &>/dev/null || true

                log_status "ok" "Preferencias XFCE de energia/screen lock aplicadas"
            else
                log_status "skip" "No se pudo resolver UID del usuario para xfconf"
            fi
        else
            log_status "skip" "xfconf-query no disponible"
        fi

        mkdir -p /etc/systemd/logind.conf.d
        cat <<'EOF' > /etc/systemd/logind.conf.d/90-omni-power.conf
[Login]
...
EOF
        ...
        log_status "ok" "Override de logind aplicado (tapa/inactividad en ignore)"

        if is_apt_installed "light-locker"; then
            run_apt_with_repair apt purge -y light-locker &>/dev/null || true
            log_status "ok" "light-locker eliminado para evitar conflictos"
        else
            log_status "skip" "light-locker no estaba instalado"
        fi
    else
        log_status "skip" "Entorno XFCE no detectado (se omiten fixes XFCE)"
    fi

    cmd_exists "update-desktop-database" && update-desktop-database &>/dev/null || true
    log_status "ok" "Base de datos de accesos de escritorio actualizada"
}

catalog_available_v1() {
...
resolve_gpu_purpose_v1() {
    local category="$1"
    if [[ "$GPU_PURPOSE" != "auto" ]]; then
        echo "${GPU_PURPOSE,,}"
        return 0
    fi
    ...
    case "$category" in
        gaming) echo "gaming" ;;
        design_graphic|design_video|design_3d) echo "design" ;;
        ai_environments) echo "ai" ;;
        *) echo "general" ;;
    esac
}

gpu_packages_for_v1() {
    local vendor="$1" purpose="$2"
...
apply_gpu_profile_for_category_v1() {
    local category="$1"
    local vendor purpose
    vendor="$(resolve_gpu_vendor_v1 "$GPU_PROFILE")"
    purpose="$(resolve_gpu_purpose_v1 "$category")"

    [[ "$vendor" == "none" ]] && {
        log_status "skip" "GPU profile omitido (sin vendor valido)"
        return 0
    }

    log_msg "Aplicando perfil GPU: vendor=${vendor} purpose=${purpose}"
    local pkg
    for pkg in $(gpu_packages_for_v1 "$vendor" "$purpose"); do
        install_with_progress "$pkg" "gpu:${vendor}:${purpose}:${pkg}"
    done

    if [[ "$vendor" == "nvidia" ]]; then
        log_msg "Nota: driver propietario NVIDIA se mantiene fuera por politica de no tocar kernel"
    fi
}

setup_ai_runtime_notes_v1() {
    local notes_file="$USER_HOME/.ai-runtime-notes.txt"
    cat <<'EOF' > "$notes_file"
Entorno AI/ML base instalado (V1).
...
4) Instalar frameworks por proyecto (torch/tensorflow/transformers) segun necesidad.

Comprobacion:
- python3 --version
- python3 -c "import numpy, pandas, sklearn; print('ok')"
EOF
    chown "$SUDO_USER:$SUDO_USER" "$notes_file"
    log_status "ok" "Guia AI creada: $notes_file"
}

process_clean_duplicates_from_catalog_v1() {
    OPERATION_EXECUTED=true
    section_header "LIMPIEZA DE DUPLICADOS (CATALOGO JSON)"

    if ! catalog_available_v1; then
        log_status "skip" "Catalogo JSON invalido/no disponible: $APP_CATALOG_JSON"
        return 0
    fi

    local cleaned=0 item
    while IFS= read -r item; do
        ...
        log_msg "[DUP] Revisando $name (preferido=${preferred_source}:${preferred_id})"

        if [[ "$preferred_source" == "flatpak" ]] && cmd_exists "flatpak" && is_flatpak_installed "$preferred_id"; then
            local apt_pkg
            while IFS= read -r apt_pkg; do
                [[ -z "$apt_pkg" ]] && continue
                if is_apt_installed "$apt_pkg"; then
                    run_apt_with_repair apt purge -y "$apt_pkg" &>/dev/null || true
                    ((cleaned++))
                fi
            done < <(echo "$item" | base64 -d | jq -r '.apt_packages[]?')
        fi

        if [[ "$preferred_source" == "apt" ]] && is_apt_installed "$preferred_id"; then
            local app_id
            while IFS= read -r app_id; do
                [[ -z "$app_id" ]] && continue
                flatpak uninstall -y "$app_id" &>/dev/null || true
                ((cleaned++))
            done < <(echo "$item" | base64 -d | jq -r '.flatpak_apps[]?')
        fi
    done < <(jq -r '.duplicates[] | @base64' "$APP_CATALOG_JSON")

    log_status "ok" "Limpieza de duplicados completada (acciones: $cleaned)"
...
configure_shell() {
    ...
    if ! grep -q "OMNI-OPTIMIZER v8.5" "$USER_HOME/.bashrc" 2>/dev/null; then
        cat <<EOF >> "$USER_HOME/.bashrc"

# === OMNI-OPTIMIZER v8.5: Configuración de shell ===
...
show_main_menu() {
    clear
    echo -e "${BLUE}╔════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}${WHITE}  DEBIAN 13 OMNI-OPTIMIZER v8.5 - MENU PRINCIPAL  ${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════════════════╝${NC}"
    ...
    echo -e "  ${GREEN}[12]${NC} Verificar por categoría"
    echo -e "  ${GREEN}[13]${NC} Limpiar duplicados (BD apps)"
    echo -e "  ${GREEN}[14]${NC} Eliminar cron de mantenimiento"
    echo -e "  ${GRAY}[c]${NC} Cancelar operación actual"
    echo -e "  ${RED}[q]${NC} Salir inmediato"
    echo -e "  ${RED}[0]${NC} Salir y finalizar"
    echo -n -e "\n${CYAN}Opción [0-14,c,q]: ${NC}"
}
...
show_module_menu() {
    ...
    local i
    for ((i=0; i<${#V1_MODULE_LABELS[@]}; i+=2)); do
        local status_l="(disponible)"
        local left_idx=$((i+1))
        is_module_installed "${V1_MODULE_IDS[$i]}" && status_l="${GREEN}(instalado)${NC}"

        local left_text
        left_text=$(printf "${GREEN}[%2d]${NC} %s %s" "${left_idx}" "${status_l}" "${V1_MODULE_LABELS[$i]}")

        if (( i + 1 < ${#V1_MODULE_LABELS[@]} )); then
            local status_r="(disponible)"
            local right_idx=$((i+2))
            is_module_installed "${V1_MODULE_IDS[$((i+1))]}" && status_r="${GREEN}(instalado)${NC}"
            local right_text
            right_text=$(printf "${GREEN}[%2d]${NC} %s %s" "${right_idx}" "${status_r}" "${V1_MODULE_LABELS[$((i+1))]}")
    ...
    echo -n -e "\n${CYAN}Categorías [00-${#V1_MODULE_IDS[@]},c,q]: ${NC}"
}
...
v1_module_id_from_selection_token() {
    local token="$1"
    local idx=$((10#$token - 1))
    [[ $idx -lt 0 || $idx -ge ${#V1_MODULE_IDS[@]} ]] && return 1
    printf '%s\n' "${V1_MODULE_IDS[$idx]}"
}

v1_module_packages_for_id() {
    local module_id="$1"
    local raw
    raw="$(v1_snapshot_category_packages "$module_id")"
    echo "$raw" | tr '\n' ' ' | sed -E 's/[\[\]",]//g; s/[[:space:]]+/ /g; s/^ //; s/ $//'
}

v1_module_flatpaks_for_id() {
    local module_id="$1"
    local raw
    raw="$(v1_snapshot_category_flatpaks "$module_id")"
    echo "$raw" | tr '\n' ' ' | sed -E 's/[\[\]",]//g; s/[[:space:]]+/ /g; s/^ //; s/ $//'
}

run_catalog_category_actions_v1() {
    local category="$1" stage="$2"
    local actions=""

    case "$stage" in
        pre)
            actions="$(catalog_get_v1_category_pre_actions "$APP_CATALOG_JSON" "$category")"
            ;;
        post)
            actions="$(catalog_get_v1_category_post_actions "$APP_CATALOG_JSON" "$category")"
            ;;
        *)
            return 0
            ;;
    ...
    for action in $actions; do
        case "$action" in
            desktop_fixes)
                ...
                ;;
            gpu_profile)
                apply_gpu_profile_for_category_v1 "$category"
                ;;
            ai_runtime_notes)
                setup_ai_runtime_notes_v1
                ;;
            dedupe_from_catalog)
                # En install/check-fix no ejecutar limpieza interactiva para evitar romper UX.
                :
                ;;
            *)
                log_status "skip" "Accion catalogo no permitida en V1: $action"
                ;;
        esac
    done
}
...
process_module_install() {
    ...
    local token idx mod pkgs_str
    for token in "${SEL[@]}"; do
        idx=$((10#$token - 1))
        
        if ! mod="$(v1_module_id_from_selection_token "$((idx+1))")"; then
        ...
        pkgs_str="$(v1_module_packages_for_id "$mod")"
        local pkgs=()
...
process_reinstall() {
    ...
    for token in "${SEL[@]}"; do
        idx=$((10#$token - 1))
        [[ $idx -lt 0 || $idx -ge ${#V1_MODULE_IDS[@]} ]] && continue

        mod="${V1_MODULE_IDS[$idx]}"
...
process_verify_category() {
    ...
    for token in "${SEL[@]}"; do
        ...
        section_header "[VERIFY-CATEGORY] ${mod}"
        local total=0 present=0 missing=0 compat_warn=0 reason

        for pkg in "${pkgs[@]}"; do
            ((total++))
            if is_apt_installed "$pkg"; then
                log_status "ok" "${pkg} presente"
                ((present++))
            else
                log_status "skip" "${pkg} faltante"
                ((missing++))
            fi

            if ! check_package_compatibility "$pkg" reason; then
                log_status "skip" "Compatibilidad ${pkg}: ${reason}"
                ((compat_warn++))
            fi
        done

        local mod_flatpaks
        mod_flatpaks="$(v1_module_flatpaks_for_id "$mod")"
        if cmd_exists "flatpak" && [[ -n "$mod_flatpaks" ]]; then
            local app
            for app in $mod_flatpaks; do
                ((total++))
                if is_flatpak_installed "$app"; then
                    log_status "ok" "Flatpak ${app} presente"
                    ((present++))
                else
                    log_status "skip" "Flatpak ${app} faltante"
                    ((missing++))
                fi
            done
        fi

        log_msg "Resumen ${mod}: presentes=${present}/${total} faltantes=${missing} compat_warn=${compat_warn}"
    done
...
process_purge() {
    ...
    # APT cleanup
    run_apt_with_repair apt autoremove -y
    run_apt_with_repair apt clean
    run_apt_with_repair apt autoclean
...
process_cleanup_obsolete() {
    ...
    for pkg in "${obsolete_pkgs[@]}"; do
        if is_apt_installed "$pkg"; then
            run_apt_with_repair apt purge -y "$pkg" && \
...
pkg_needed_by_other_installed_categories() {
    ...
    local i
    for i in "${!V1_MODULE_IDS[@]}"; do
        local mod="${V1_MODULE_IDS[$i]}"
        ...
        local pkgs_str
        pkgs_str="$(v1_module_packages_for_id "$mod")"
        [[ -n "$pkgs_str" ]] && read -ra other_pkgs <<< "$pkgs_str"
...
flatpak_needed_by_other_installed_categories() {
    ...
    local category
    for category in "${V1_MODULE_IDS[@]}"; do
        [[ "$category" == "$current" ]] && continue
        is_module_installed "$category" || continue
        local apps
        apps="$(v1_snapshot_category_flatpaks "$category")"
        for app in $apps; do
            [[ "$app" == "$app_id" ]] && return 0
        done
    done
...
process_remove_category() {
    ...
    for token in "${SEL[@]}"; do
        ...
        for pkg in "${pkgs[@]}"; do
            pkg_needed_by_other_installed_categories "$pkg" "$mod" && {
                log_status "skip" "$pkg conservado (usado por otra categoría instalada)"
                continue
            }
            if is_apt_installed "$pkg"; then
                run_apt_with_repair apt purge -y "$pkg" && log_status "ok" "Purgado: $pkg"
            fi
        done

        local mod_flatpaks
        mod_flatpaks="$(v1_snapshot_category_flatpaks "$mod")"
        if cmd_exists flatpak && [[ -n "$mod_flatpaks" ]]; then
            for app in $mod_flatpaks; do
...
process_updates_and_cron() {
    ...
    local cron_log_file="${LOG_DIR}/debian-postinstall-cron.log"
    cat <<'EOF' > "$cron_script"
#!/usr/bin/env bash
set -o errexit
set -o nounset
...
process_remove_updates_cron() {
    OPERATION_EXECUTED=true
    section_header "ELIMINAR CRON DE MANTENIMIENTO"

    local cron_file="/etc/cron.d/debian-postinstall-maintenance"
    local cron_script="/usr/local/bin/debian-postinstall-maintenance.sh"

    if [[ ! -f "$cron_file" && ! -f "$cron_script" ]]; then
        log_status "skip" "No se encontraron artefactos de cron V1 para eliminar"
        echo -e "${CYAN}Presione ENTER para continuar...${NC}"
        read -r
        return 0
    fi

    rm -f "$cron_file" "$cron_script"
    systemctl reload cron &>/dev/null || true

    log_status "ok" "Cron de mantenimiento V1 eliminado"
    echo -e "${CYAN}Presione ENTER para continuar...${NC}"
    read -r
}
...
show_final_dashboard() {
    ...
    clear
    echo -e "${BLUE}╔════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}${WHITE}  INSTALACIÓN COMPLETADA - OMNI-OPTIMIZER v8.5  ${NC}"
    echo -e "${BLUE}╚════════════════════════════════════════════════════╝${NC}"
                ...
                while true; do
                    ...
                    case "$sel" in
                        ...
                        q|Q)
                            ...
                            return
                            ;;
                        *) process_verify_category "$sel" ;;
                    esac
                done
                ;;
            13) process_clean_duplicates_from_catalog_v1 ;;
            14) process_remove_updates_cron ;;
            0)
                post_install_hooks
                show_final_dashboard
                break
...
main() {
    ...
    trap 'finalize_run_log_v1 "$?"' EXIT

    if declare -F catalog_require_ready_or_exit >/dev/null; then
        catalog_require_ready_or_exit "$APP_CATALOG_JSON" "post-install.sh"
    fi
    ...
    elif [[ -f "$AUTO_PROFILE_JSON" ]] && declare -F json_profile_audit >/dev/null; then
        section_header "[JSON] Auditoría preliminar del snapshot previo"
        json_profile_audit "$AUTO_PROFILE_JSON" "$USER_HOME"
    fi
    
    # Banner de inicio
    clear
    echo -e "${BLUE}╔════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}${WHITE}  DEBIAN 13 OMNI-OPTIMIZER v8.5                 ${NC}"
    ...
    if [[ "${OPERATION_EXECUTED}" == "true" ]]; then
        update_auto_profile_snapshot_v1
    else
        log_status "skip" "Snapshot omitido: no se ejecutaron operaciones"
    fi
