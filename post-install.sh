...
v1_snapshot_category_packages() {
    local category="$1"
    case "$category" in
        optimization) echo "zram-tools earlyoom preload tlp thermald irqbalance" ;;
        browsers) echo "${MODULE_BROWSERS[*]}" ;;
        office) echo "${MODULE_OFFICE[*]}" ;;
        multimedia) echo "${MODULE_MULTIMEDIA[*]}" ;;
        dev_core) echo "${MODULE_DEV_CORE[*]}" ;;
        dev_cli) echo "${MODULE_DEV_CLI[*]}" ;;
        dev_web) echo "${MODULE_DEV_WEB[*]}" ;;
        dev_containers) echo "${MODULE_DEV_CONTAINERS[*]}" ;;
        dev_mobile) echo "${MODULE_DEV_MOBILE[*]}" ;;
        communication) echo "${MODULE_COMMUNICATION[*]}" ;;
        virtualization) echo "${MODULE_VIRTUALIZATION[*]}" ;;
        hardware_drivers) echo "${MODULE_HARDWARE_DRIVERS[*]}" ;;
        vpn_free) echo "${MODULE_VPN_FREE[*]}" ;;
        design_graphic) echo "${MODULE_DESIGN_GRAPHIC[*]}" ;;
        design_video) echo "${MODULE_DESIGN_VIDEO[*]}" ;;
        design_3d) echo "${MODULE_DESIGN_3D[*]}" ;;
        gaming) echo "${MODULE_GAMING_NATIVE[*]}" ;;
        windows_compat) echo "${MODULE_WINDOWS_COMPAT[*]}" ;;
        cybersec) echo "${MODULE_CYBERSEC[*]}" ;;
        remote) echo "${MODULE_REMOTE[*]}" ;;
        monitoring) echo "${MODULE_MONITORING[*]}" ;;
        backup) echo "${MODULE_BACKUP[*]}" ;;
        security) echo "${MODULE_SECURITY[*]}" ;;
        *) echo "" ;;
    esac
}
...
update_auto_profile_snapshot_v1() {
    ...
    local -a snapshot_categories=(optimization browsers office multimedia dev_core dev_cli dev_web dev_containers dev_mobile communication virtualization hardware_drivers vpn_free design_graphic design_video design_3d gaming windows_compat cybersec remote monitoring backup security)
